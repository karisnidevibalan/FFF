    ...
    const handleAnalyze = async () => {
        ...
        try {
            for (let i = 0; i < files.length; i++) {
                ...
                const extractRes = await fetch(`${API_URL}/ai/extract-text`, {
                    method: 'POST',
                    body: formData,
                });

                if (!extractRes.ok) throw new Error(`Failed to extract text from ${file.name}`);
                const { text } = await extractRes.json();

                // 2. Analyze Candidate
                const analyzeRes = await fetch(`${API_URL}/ai/analyze-candidate`, {
                    method: 'POST',
                    headers: {
                    ...
                    body: JSON.stringify({
                        resumeText: text,
                        jobRole: 'Candidate', // Default role
                ...
                if (analyzeResult.success) {
                    newResults.push({
                        id: i,
                        name: analyzeResult.data.name || file.name.replace(/\.[^/.]+$/, ""),
                        email: analyzeResult.data.email || 'N/A',
                        score: analyzeResult.data.matchScore || 0,
                        match: analyzeResult.data.matchScore >= 80 ? 'High Match' : 'Medium Match',
                        skills: analyzeResult.data.skills?.slice(0, 3) || [],
                        status: 'Analyzed'
                    });
                }
            }
            ...
            setStep(3);
            toast.success(`Analysis complete! ${newResults.length} resumes processed.`);
        } catch (error: any) {
